...
census_api_key(key="616907b994896c1dde3f6ba7e408703e51e9fd26", install= TRUE, overwrite = T)
...
{
  get_decennial(geography = "block", variables = varsP12$name, year = y, state = 17, county = x)
...
{
  get_acs(geography = "tract", variables = varsB1001$name, survey="acs5", year = y, state = 17, county= x)
  
}

load_illinois_blocks_by_county <- function (counties=seq(1,203,2), year=2010, cutoff = 1, postprocess = T ) 
{
  
  # in current version, this loads illinois block level p12 data, from the 2010 census
  # by county (default all, otherwise list), by default it only returns rows with a value
  # of 1 or higher, meaning at least 1 person per row.
  
  # it is resistant to basic errors in the API and will always return a list of any failed arguments
  # based on the actual exported file
  
  output <- foreach(county=counties,.errorhandling="remove", .final = rbindlist) %do% {
    
    data.table(get_p12_blocks_by_county(county, year))[value>=cutoff,c(1,3,4)]
    }
 
  print("Failed counties, in FIPS code:")
  print(counties[!(counties %in% as.numeric(unique(output[,substr(GEOID,3,5)])))])

  if (postprocess) {
    output[,.(GEOID,value,gender=gender_of_p(variable),race=ethnicity_of_p(variable),age=age_of_p(variable))]
  } else  {output}
}

load_illinois_acs_by_county <- function (counties=seq(1,203,2), year=2018, cutoff = 1, postprocess = T ) 
{
  ...
  output <- foreach(county=counties,.errorhandling="remove", .final = rbindlist) %do% {
    ...
    data.table(get_acs_tract_by_county(county,year))[estimate>=cutoff,c(1,3,4)]
    }
  ...
  if (postprocess) {
    output[,.(GEOID,estimate,gender=gender_of_b(variable),race=ethnicity_of_b(variable),age=age_of_b(variable))]
  } else  {output}
}
